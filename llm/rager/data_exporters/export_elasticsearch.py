...
def elasticsearch(
    ...
    # index_name = kwargs.get('index_name', 'documents') # REPLACED
    index_name_prefix = kwargs.get('index_name', 'documents')
    current_time = datetime.now().strftime("%Y%m%d_%M%S")
    index_name = f"{index_name_prefix}_{current_time}"
    print("index name:", index_name)

    # we need index_name variable on anothers blocks
    set_global_variable('rag_llmzoomcamp', 'index_name', index_name)
